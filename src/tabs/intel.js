...
function _debugIntelNews() {
  ...
  // Dump AppState structure
  try {
    const AS = JSON.parse(localStorage.getItem('AppState') || '{}');
    console.log('[IntelDebug] AppState top-level keys:', Object.keys(AS));
    if (AS.kingdomNews) console.log('[IntelDebug] AppState.kingdomNews keys:', Object.keys(AS.kingdomNews));
    // Deep search AppState for news text
    function _deepDump(obj, path, depth) {
      if (depth > 4 || !obj || typeof obj !== 'object') return;
      for (const [k, v] of Object.entries(obj)) {
        const p = `${path}.${k}`;
        if (typeof v === 'string' && v.length > 30) {
          console.log(`[IntelDebug] String at ${p} (len=${v.length}):`, v.slice(0,300));
        } else if (v && typeof v === 'object' && !Array.isArray(v)) {
          console.log(`[IntelDebug] Object at ${p}, keys:`, Object.keys(v).slice(0,15));
          _deepDump(v, p, depth + 1);
        } else if (Array.isArray(v)) {
          console.log(`[IntelDebug] Array at ${p}, length:`, v.length, v[0] ? JSON.stringify(v[0]).slice(0,100) : '');
        }
      }
    }
    _deepDump(AS, 'AppState', 0);
  } catch(e) { console.log('[IntelDebug] AppState parse error:', e.message); }
