...
function calcAttacks(prov) {
  ...
  // Own pop% — calculated from IS fields per Utopia formula:
  //   Current Population = peasants + totalTroops + thieves + wizards
  //   Living Space (simplified) = land * 25  (assumes all built, no homes/bonuses)
  //   Pop% = Current Population / Living Space * 100
  // sot.ppa on own provinces is peasants-per-acre only, not total people — don't use it.
  const _sot = prov.sot || {};
  const _land = prov.land || _sot.land || 0;
  const _totalPop = (_sot.peasants || 0) + (_sot.totalTroops || 0)
                  + (_sot.thieves  || 0) + (_sot.wizards    || 0);
  const ownPop = _land > 0
    ? Math.min(Math.round(_totalPop / (_land * 25) * 100), 150)
    : null;
  // Debug — remove once confirmed correct: console.log('[WP pop%]', {peasants:_sot.peasants,totalTroops:_sot.totalTroops,thieves:_sot.thieves,wizards:_sot.wizards,land:_land,ownPop});
