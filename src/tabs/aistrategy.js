...
async function aiStrategySendToDiscord() {
  ...
  // Discord embed description limit is 4096 chars — split into chunks if needed
  const CHUNK = 4000;
  const text = r.analysis || '';
  const chunks = [];
  let pos = 0;
  while (pos < text.length) {
    // Try to break at a newline near the limit so we don't cut mid-sentence
    let end = pos + CHUNK;
    if (end < text.length) {
  ...
  for (let i = 0; i < total; i++) {
    ...
    const ok = await sendDiscordEmbed(S.discordWebhook, {
      content: isFirst ? '' : '',
      embeds: [{
        title: isFirst ? `🧠 AI Strategy — vs ${S.eLoc || '?'}${total > 1 ? ` (${i+1}/${total})` : ''}` : `🧠 AI Strategy — vs ${S.eLoc || '?'} (${i+1}/${total})`,
        description: chunks[i],
        color: 4886754,
        timestamp: new Date().toISOString(),
        footer: isLast ? { text: `Generated ${r.generated_at || ''}${r.calls_today ? ` · ${r.calls_today}/${r.daily_limit} calls today` : ''}` } : undefined,
      }],
    });
    if (!ok) { failed = true; break; }
    if (i < total - 1) await new Promise(res => setTimeout(res, 500));
  }
  if (failed) alert('Failed to send to Discord — check the webhook URL.');
}
